...
- Phase 6 local AI portability and daily-use integration is in progress through IMP-083;
...
- IMP-074 adds one explicit bounded read of a caller-selected regular non-symlink UTF-8 text or Markdown file, with deterministic BOM handling, exact hashes, fixed external-content origin, and no persistence, model execution, or network access;
- IMP-074 is assigned to Issue #231;
- the IMP-074 local-document extension passes at the `ci` evidence level and does not broaden accepted real-machine evidence;
...
- IMP-077 adds one explicit bounded local OCR path for a caller-selected PNG or JPEG image, with standard-library source validation and an invocation-only optional macOS Vision adapter;
- IMP-077 is assigned to Issue #237;
- the IMP-077 local-image OCR extension passes at the `ci` evidence level, including hosted macOS Vision adapter execution, and does not broaden accepted primary real-machine evidence;
- IMP-078 composes one explicit local text or Markdown document into revise, summarize, and translate as the primary data-only writing source while preserving inline-source behavior and the existing task-authority boundary;
- IMP-078 is assigned to Issue #239;
...
- IMP-083 adds a bounded local-only Lite client resource measurement harness with a fixed five-step workload, monotonic timing, doll-process RSS, bounded workspace disk counts, privacy-safe JSON evidence, exact tracked-checkout guards, and Ubuntu/macOS/Windows synthetic acceptance;
- IMP-083 is assigned to Issue #249;
- the IMP-083 Lite client resource measurement harness passes at the `ci` evidence level and does not define resource thresholds or establish primary Intel Mac real-machine resource evidence;
- the next bounded implementation receives IMP-084 only when a new implementation issue is opened;
...
Implemented at most one caller-selected external Resume Bundle path for the accepted local `draft`, `revise`, and `summarize` workflow. The existing Resume Bundle v1 verifier runs before any origin or runtime request, and the selected file is bound to a content-free SHA-256 identity without exposing its native path.

Only the bounded core continuity groups enter one deterministic snapshot: project, checkpoint, active/next/blocked work, decisions, procedures, policies, and pending validation. HANDOFF text, checksum rows, artifact/source reference rows, artifact bytes, external source content, and linked-record expansion remain excluded.

The snapshot is secret-scanned, shares the existing ten-item and 24,000-character limits, persists at no lower than sensitive classification, and enters only as data-only external content through extractor/extraction. Missing, unreadable, tampered, symlinked, unsupported, oversized, secret-bearing, changed-during-read, or over-limit bundles fail before runtime and before context-origin creation.

The content-free result exposes only project ID, source state revision, bundle SHA-256, member-group count, and character counts. Dedicated acceptance covers all three writing modes, authority denial, excluded-member proof, failure preservation, aggregate limits, path privacy, malformed input, and coverage of fail-closed branches. Standard CI covers Ubuntu, macOS, and Windows.
...
Status: implemented with deterministic synthetic cross-platform CI evidence and hosted macOS Vision adapter evidence.

Implemented `doll ocr extract` over one caller-selected regular non-symlink PNG or JPEG image through a replaceable optional adapter contract. Source bytes are bounded to eight megabytes and protected by path/open-handle identity, size, and modification-time verification. Standard-library validation checks PNG chunk structure and CRCs or bounded JPEG marker structure, rejects animated PNG input, requires extension/content agreement, and enforces 10,000-pixel width and height limits plus a 25,000,000-pixel decoded-area limit before OCR execution.

The first real adapter is `ocrmac 1.0.1` over Apple Vision and is imported only when OCR is invoked on macOS. The lock resolves Pillow 12.3.0 and matching PyObjC 12.2.1 framework packages for that optional path. Core startup, help, non-OCR commands, Windows, and Ubuntu do not require the OCR stack. OCR receives validated in-memory image data rather than a path or URL and performs no process launch, shell execution, network access, cloud access, or automatic dependency or model download.

Recognized output is bounded to 1,000 ordered lines, 20,000 characters per line, and 200,000 aggregate characters. Empty recognition is a successful explicit result. Human-readable and deterministic JSON output are available, including metadata-only mode. Every recognized line remains `external_content`, actor `extractor`, acquisition `ocr`, authority `untrusted_data`, and is not injected automatically into model context.

Dedicated acceptance covers PNG and JPEG validation, Unicode and Japanese adapter-return preservation, hashes and dimensions, empty results, adapter absence, malformed and changed inputs, symlink and limit rejection, bounded output, path privacy, exact source/workspace/state preservation, and the default real `ocrmac`/Vision path on hosted macOS. Standard CI covers Ubuntu, macOS, and Windows. Hosted macOS adapter execution is CI evidence and does not replace or broaden primary Intel Mac real-machine release evidence.

IMP-077 does not establish scanned-PDF OCR, automatic fallback from PDF extraction, multipage or animated image support, handwriting tuning, layout or table reconstruction, bounding-box publication, image enhancement, attachment integration, persistent OCR results, artifact publication, persistent indexing, semantic retrieval, model-selected context, Web retrieval, cross-platform real OCR adapters, performance acceptance, accessibility presentation, the release-candidate soak, complete Phase 6, Lite v1.0 completion, or stable general anti-lock-in.

### IMP-078 — Explicit text and Markdown writing attachments

Status: implemented with deterministic synthetic CI evidence.

Extended the accepted local writing workflow with one optional caller-selected `.txt`, `.md`, or `.markdown` source document. `revise`, `summarize`, and `translate` require exactly one primary source form: existing inline `source_text` or one explicit local document. `draft` remains source-free. Existing inline-source behavior is preserved.

The workflow validates mode, request, source-form selection, target language, operation identity, conversation target, active binding, and runtime declaration before opening the selected file. The document is then read only through the unchanged IMP-074 bounded reader, including regular-file and symlink checks, one-megabyte byte limit, strict UTF-8, deterministic BOM handling, binary/control rejection, and path/open-handle identity, size, and modification-time verification. The existing 16,000-character writing-source limit is applied before any source InstructionOrigin is created.

Validated document text enters only as data-only `external_content` through `extractor` / `extraction` with `untrusted_data` authority. The current user request remains the sole task-authority instruction. Hostile document instructions cannot change writing mode, target language, permissions, confirmation, capability authority, binding state, memory, project, decision, or completion authority. The source remains subject to the existing prompt-injection boundary.

The content-free writing result identifies `none`, `inline`, or `document` source kind. Document-source metadata contains only document kind, source byte count, source SHA-256, returned-content SHA-256, BOM-removal state, and the existing writing-source character count. No native path, filename, source text, prompt text, response text, credential, or secret value is added to the result.

IMP-078 creates no new persistent document or SourceRecord, copies no source file, creates no artifact or file-content index, performs no automatic discovery, directory traversal, globbing, semantic retrieval, model-selected context, process or shell execution, capability execution, network or cloud access, credential access, or binding change. Dedicated acceptance covers Markdown/BOM input, exact metadata, unchanged inline behavior, source-form conflicts, target-before-read ordering, invalid document failure before source-origin creation, writing limits, hostile document treatment, runtime failure, and exact source-file preservation. Standard CI covers Ubuntu, macOS, and Windows.

IMP-078 does not establish PDF, OCR/image, or CSV writing attachments, multiple attachments, draft reference attachments, attachment persistence, source-record creation, artifact publication, persistent indexing, automatic file discovery, semantic retrieval, embeddings, ranking, model-selected context, Web retrieval, cloud inference, tools, target-specific export, accessibility presentation, Lite performance acceptance, the release-candidate soak, primary Intel Mac evidence for this extension, complete Phase 6, Lite v1.0 completion, or stable general anti-lock-in.
...
Status: measurement harness mechanics implemented with deterministic synthetic cross-platform CI evidence.

IMP-083 establishes a bounded measurement path for doll Lite client overhead without guessing minimum or recommended hardware requirements. The fixed local-only workload measures five steps in order: workspace initialization, authoritative state initialization, workspace reload, immutable read-only state open, and read-only `doll doctor`. Every step and the total workload use monotonic nanosecond timing.

The harness records only doll-process RSS and aggregate workspace disk usage. Linux may report current RSS from `/proc/self/statm` plus peak RSS from `resource.getrusage`; macOS reports normalized peak RSS through `resource.getrusage`; Windows uses `GetProcessMemoryInfo` through in-process `ctypes`. Workspace traversal is bounded to 20,000 entries and depth 32, rejects an existing root symlink before initialization, does not follow inner symlinks, rejects root escape and unsupported entries, and returns only aggregate byte, file, and directory counts.

The evidence runner requires the supplied 40-character commit SHA to equal `git rev-parse HEAD`, separately requires the tracked index to match HEAD and the tracked working tree to match the index, and therefore rejects staged or unstaged tracked source changes before measurement. Untracked evidence output remains allowed so shell redirection does not invalidate the source guard. A Python audit hook rejects measured-workload network connection attempts and process launches while keeping the evidence-wrapper Git checks outside the measured workload.

Shareable JSON excludes native paths, workspace identifiers, usernames, hostnames, model names, request/source/prompt/response text, credentials, and secrets. Dedicated synthetic acceptance passes on Ubuntu, macOS, and Windows, including the Windows process-memory-counter path. Standard CI remains subject to the unchanged 95 percent project coverage threshold. A primary Intel Mac real-machine runbook exists, but no primary Intel Mac resource result is accepted by IMP-083.

IMP-083 does not define RAM, disk, or latency thresholds; minimum or recommended hardware; Ollama or other external runtime memory; model-weight memory; GPU or VRAM use; model response latency or throughput; total-system resource use; Heavy measurements; primary Intel Mac resource acceptance; Lite performance acceptance; the release-candidate soak; complete Phase 6; Lite v1.0 completion; or stable general anti-lock-in.

Subsequent daily-use work may expand cross-platform OCR adapters, accessibility presentation, primary Intel Mac Lite resource measurements and threshold interpretation, and soak testing.
