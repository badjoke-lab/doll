"""Verified backup restore and fresh-process post-restore validation."""
...
def restore_state_backup(backup_path: Path, target: Path) -> RestoreResult:
    """Restore one verified state backup into an absent or empty target."""
...
def restore_workspace_backup(backup_path: Path, target: Path) -> RestoreResult:
    """Restore one verified workspace backup into an absent or empty target."""
...
def validate_restored_workspace(
    ...
    *,
    ...
    """Open a restored workspace read-only and validate its full portable contract."""
    ...
    if workspace.record.state_revision != expected_state_revision:
        raise RestoreValidationError("restored workspace revision does not match backup")

    descriptor, package_name = tempfile.mkstemp(prefix=".doll-restore-check-", suffix=".zip")
    os.close(descriptor)
    ...
    try:
        with open_state_repository(target, read_only=True) as repository:
            ...
            if status.state_revision != expected_state_revision:
                raise RestoreValidationError("restored database revision does not match backup")
            ...
            audit_row = repository.connection.execute("SELECT COUNT(*) FROM audit_events").fetchone()
            backup_row = repository.connection.execute(
            ...
            if audit_row is None or backup_row is None:
                raise RestoreValidationError("restored inventories are unreadable")
        ...
        if verified.workspace_id != expected_workspace_id:
            raise RestoreValidationError("portable validation identity changed")
        if verified.schema_version != expected_schema_version:
            raise RestoreValidationError("portable validation schema changed")
        if verified.state_revision != expected_state_revision:
            raise RestoreValidationError("portable validation revision changed")
        ...
        return RestoreValidation(
            ...
            backup_inventory_count=int(backup_row[0]),
            audit_event_count=int(audit_row[0]),
        )
    except RestoreError:
        raise
...
def _restore_backup(backup_path: Path, target: Path, *, expected_kind: str) -> RestoreResult:
    ...
    try:
        if expected_kind == "state":
            _stage_state_restore(members, staging)
        else:
        ...
        staged = validate_restored_workspace(
            ...
            expected_state_revision=inspection.source_state_revision,
        )
        empty_backup = _publish_target(staging, target_path, target_existed=target_existed)
        published = True
        fresh = _validate_in_fresh_process(target_path, inspection)
        if fresh != staged:
...
def _prepare_target(target: Path) -> tuple[Path, bool]:
    ...
    if target_existed:
        ...
        try:
            ...
            raise RestoreConflictError("restore target cannot be inspected") from exc
    parent = target_path.parent
    parent.mkdir(parents=True, exist_ok=True)
    if not parent.is_dir():
        raise RestoreConflictError("restore target parent is not a directory")
    return target_path, target_existed
...
def _read_verified_members(
    ...
    if len(content) != inspection.file_size_bytes:
        raise BackupIntegrityError("backup changed after verification")
    if f"sha256:{hashlib.sha256(content).hexdigest()}" != inspection.file_sha256:
        raise BackupIntegrityError("backup changed after verification")
    try:
...
def _stage_state_restore(members: dict[str, bytes], staging: Path) -> None:
    try:
        nested = members[_STATE_MEMBER]
    except KeyError as exc:
        raise BackupIntegrityError("state backup payload is missing") from exc
    descriptor, nested_name = tempfile.mkstemp(
    ...
    try:
        ...
        import_state_package(nested_path, staging)
...
def _stage_workspace_restore(members: dict[str, bytes], staging: Path) -> None:
    ...
    try:
        ...
        for member_name, content in members.items():
            if not member_name.startswith(_ARTIFACT_PREFIX):
                continue
            managed = member_name.removeprefix(_ARTIFACT_PREFIX)
            safe_path = validate_managed_path(managed)
            destination = staging / "artifacts" / safe_path
...
def _publish_target(staging: Path, target: Path, *, target_existed: bool) -> Path | None:
    ...
    try:
        ...
        if empty_backup is not None and empty_backup.exists() and not target.exists():
...
def _validate_in_fresh_process(
    target: Path, inspection: BackupInspection
) -> RestoreValidation:
    command = [
        sys.executable,
        "-m",
        ...
        "--state-revision",
        str(inspection.source_state_revision),
    ]
    try:
