...
- IMP-001 through IMP-023 and IMP-030 through IMP-089 are complete on this implementation branch; IMP-024 through IMP-029 are retired and must not be reused.
...
- Persistent memory-use signals, supported/default semantic retrieval, persistent embeddings, hybrid fusion, automatic context injection, PAM/PLUR/PROJECTMEM adapters, ProjectExperienceRecord, ContinuityPreflight, MCP, cloud expansion, and State Package signing are not established by IMP-089 and must follow their accepted specification, safety, portability, usefulness-evidence, and acceptance boundaries.
