...
def test_validation_requirement_short_circuits_nonblocking_states(
    ...
    with state.open_state_repository(initialized.root) as repository:
        ...
        exempt = service.create(
            project_id=project_id,
            kind="task",
            title="Not-applicable validation fixture",
            description="Not-applicable work does not remain pending.",
            acceptance_criteria=(
                AcceptanceCriterion("blocking", "Blocking criterion.", None, True),
            ),
        )
        exempt = service.set_verification(
            exempt.work_item_id,
            expected_revision=exempt.revision,
    ...
    assert _requires_validation(exempt) is False
