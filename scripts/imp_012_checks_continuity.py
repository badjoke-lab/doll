...
def evaluate(data: dict[str, Any]) -> dict[str, bool]:
    return _core(data) | _recovery(data)


def _core(data: dict[str, Any]) -> dict[str, bool]:
    source = data["source"]
    restart = data["restart"]
    return {
        "CONT-P001_workspace_initialization": source["status"].workspace_id
...
def _recovery(data: dict[str, Any]) -> dict[str, bool]:
    restored_state = data["restored_state"]
    restored_workspace = data["restored_workspace"]
    audit_count = data["source"]["audit_count"]
    return {
        ...
        == data["state_backup"].inspection.file_sha256
        and data["workspace_inspection"].file_sha256
        == data["workspace_backup"].inspection.file_sha256,
