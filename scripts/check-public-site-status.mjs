...
expect(
  status.model_runtime.message.includes("through IMP-094") &&
    ...
    status.model_runtime.message.includes("optional rebuildable exact-token SQLite sidecar") &&
    status.model_runtime.message.includes("temporary/recall-index") &&
    status.model_runtime.message.includes("fails closed when missing, corrupt, unsupported, wrong-workspace, or stale") &&
    status.model_runtime.message.includes("versioned fabricated benchmark") &&
    status.model_runtime.message.includes("six lexical cases at recall@1, recall@3, and MRR of 1") &&
    status.model_runtime.message.includes("two low-overlap paraphrase cases as explicit semantic opportunities") &&
    status.model_runtime.message.includes("does not add embeddings, semantic/vector retrieval") &&
    status.model_runtime.message.includes("bounded local semantic recall candidate harness") &&
    status.model_runtime.message.includes("fixed IPv4 loopback Ollama transport") &&
    status.model_runtime.message.includes("POST /api/embed") &&
    status.model_runtime.message.includes("explicit already-installed non-cloud model name") &&
    status.model_runtime.message.includes("synthetic CI evidence without downloading model weights") &&
    ...
    status.model_runtime.message.includes("version-1 append-oriented ProjectExperienceRecord") &&
    status.model_runtime.message.includes("attempt, outcome, resolution, and lesson") &&
    status.model_runtime.message.includes("explicit user/system/import/model assertion and provenance") &&
    status.model_runtime.message.includes("linked replacement rather than silent semantic rewrite") &&
    status.model_runtime.message.includes("State Package export/import, backup/restore, compatibility, and fresh-process inspection") &&
    status.model_runtime.message.includes("Resume Bundle v1 reports deterministic omission") &&
