"""Run the IMP-037 Phase 4A portability acceptance evidence check."""
...
def _accepted_machine_result(matrix: dict[str, Any], gate: dict[str, Any]) -> bool:
    if gate.get("status") == "pending":
        ...
        return False
    if gate.get("status") != "pass" or matrix.get("phase4a_gate_complete") is not True:
...
def _matrix_evidence() -> tuple[dict[str, bool], list[str], bool]:
    ...
    for item in entries:
        ...
        if (
            ...
            raise RuntimeError("invalid portability test evidence")
        if (
    ...
    checks = {
        ...
        "fresh_inspector_standard_library_only": _inspector_is_independent(),
    }
    return checks, limitations, stored_complete
